...
function Home() {
  ...
  useEffect(()=> {
    fetch('https://my-moviz-backend-xi-rosy.vercel.app/movies') // http://localhost:3000/movies
  ...
  */
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.length > 250 ? `${data.overview.substring(0, 250)}...` : data.overview} poster={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
